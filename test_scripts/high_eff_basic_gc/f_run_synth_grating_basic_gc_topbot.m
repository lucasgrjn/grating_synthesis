function [] = f_run_synth_grating_basic_gc_topbot( lambda, optimal_angle, layer_thick )
% authors: bohan
% 
% script for testing the latest synthesis object on the scc
% using the basic GC dimensions
%
% running for top versus bottom fill
%
% Inputs
%   lambda
%       wavelength in nm
...
% design space fills
fill_bots  = fliplr( 0.90:0.025:0.95 );
fill_tops  = fliplr( 0.90:0.025:0.95 );
% fill_bots           = fliplr( 0.95:0.025:0.975 );
% fill_top_bot_ratio  = fliplr( 0.95:0.025:0.975 );

% run design space generation
tic;
synth_obj = synth_obj.generate_design_space_filltopbot( fill_bots, fill_tops );
toc;
