...
% waveguide
% sweep discretization and show numerical error convergence
...
% pick discretizations
disc_all = [ 100, 50, 20, 10, 5, 2 ];

% init saving variables
k_all       = zeros(size(disc_all));
k_all_old   = zeros(size(disc_all));

for ii = 1:length(disc_all)
...
% plot results
figure;
plot( disc_all, k_all, '-o' ); hold on;
plot( disc_all, k_all_old, '-o' );
plot( xlim, [ k_analytical, k_analytical ], '--' );
legend('modesolver k', 'old modesolver k', 'analytical k');
xlabel('discretization (nm)');
title('Numerical error convergence vs. discretization size');
makeFigureNice();

% plot results
figure;
plot( disc_all, 100*abs(k_all-k_analytical)./k_all, '-o' ); hold on;
plot( disc_all, 100*abs(k_all_old-k_analytical)./k_all_old, '-o' );
% plot( xlim, [ k_analytical, k_analytical ], '--' );
% legend('% error', 'analytical k');
legend('new modesolver', 'old modesolver');
xlabel('discretization (nm)'); ylabel('% error');
title('Numerical error convergence vs. discretization size');
makeFigureNice();
