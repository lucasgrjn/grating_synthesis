...
% waveguide
% testing whether solver is invariant to period
...
% sweep periods
period_all = [ 10, 50, 100, 150, 200, 250, 500 ];

% init saving variables
k_all       = zeros(size(period_all));
k_all_old   = zeros(size(period_all));

for ii = 1:length(period_all)
    % for each discretization
    
    fprintf('loop %i of %i\n', ii, length(period_all) );
    
    period = period_all(ii);
    domain = [ 1000, period ] ;
...
% plot results
figure;
plot( period_all, k_all, '-o' ); hold on;
% plot( period_all, k_all_old, '-o' );
plot( xlim, [ k_analytical, k_analytical ], '--' );
% legend('modesolver k', 'old modesolver k', 'analytical k');
legend('modesolver k', 'old modesolver k', 'analytical k');
xlabel('period (nm)');
title('Numerical error convergence vs. period');
makeFigureNice();

% plot results
figure;
plot( period_all, 100*abs(k_all-k_analytical)./k_all, '-o' ); hold on;
% plot( period_all, 100*abs(k_all_old-k_analytical)./k_all_old, '-o' );
% plot( xlim, [ k_analytical, k_analytical ], '--' );
% legend('% error', 'analytical k');
% legend('new modesolver', 'old modesolver');
xlabel('period (nm)'); ylabel('% error');
title('Numerical error convergence vs. period');
makeFigureNice();
