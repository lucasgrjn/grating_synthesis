...
for ii = 1:length(k0_all)
    ...
    % [Phi_all, k_all, A, B] = complexk_mode_solver_2D_PML_old( GC.N, ...
    %                                                            disc, ...
    %                                                            k0_all(ii), ...
    %                                                            num_modes, ...
    %                                                            guessk, ...
...
% plot the bandstructure for ALL modes, new solver
% im = imag(all_k) + pi .* power(-1, int(imag(all_k) / pi));
% re = power(-1, floor(real(all_k)*period/pi)) .* (real(all_k)*period/pi - floor(real(all_k)*period/pi));
% re = (real(all_k)*period/pi - floor(real(all_k)*period/pi));
val = real(all_k)*period/pi;
rnd = sign(val).*(floor(abs(val)));
re = (val - rnd);
re_reshaped = [];
for kk = 1:length(re)
    if ne(mod(rnd(kk),2), 0)
        re_reshaped = [re_reshaped; re(kk) - sign(re(kk))];
    else
        re_reshaped = [re_reshaped; re(kk)];
    end
end
figure;
% plot( real(all_k)*period/pi, all_k0*period, 'o', 'MarkerEdgeColor', 'green'); hold on;
% plot( re, all_k0*period, 'o', 'MarkerEdgeColor', 'red'); hold on;
plot( re_reshaped, all_k0*period, 'o', 'MarkerEdgeColor',[0 0.4470 0.7410] ); hold on;
plot( imag(all_k)*period/pi, all_k0*period, 'o','MarkerEdgeColor', 	[0 0 0] ); hold on;
xlabel('ka/pi'); ylabel('k0*a/pi');
legend('real', 'imag', 'center of bandgap');
title('Bandstructure of all solved modes, new ver.');
makeFigureNice();
